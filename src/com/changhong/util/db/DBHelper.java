...
public class DBHelper extends SQLiteOpenHelper
{
	...
	//cxp创建数据库表
	private void createOrUpdate(SQLiteDatabase db){
		
		try {
			String sqlString = DBUtils.creatTableSql(Fruit.class);
			db.execSQL(sqlString);
			
			String sqlString2 = DBUtils.creatTableSql(FruitEat.class);
			db.execSQL(sqlString2);
			CHLogger.v(TAG, "==========create ok========");
